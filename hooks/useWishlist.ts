...
export const useWishlist = (): UseWishlistReturn => {
  ...
  // Thêm vào wishlist - luôn lưu vào database
  const addToWishlist = useCallback(async (productId: number, productData?: any) => {
    setIsLoading(true);
    try {
      // Luôn lưu vào database, dù đã đăng nhập hay chưa
  ...
  // Xóa khỏi wishlist - luôn xóa từ database
  const removeFromWishlist = useCallback(async (productId: number) => {
    setIsLoading(true);
    try {
      // Luôn xóa từ database, dù đã đăng nhập hay chưa
  ...
  const syncWishlist = useCallback(async () => {
    ...
    try {
      ...
      for (const item of localWishlist) {
        try {
          await axios.post('/api/wishlist', { productId: item.product.id });
        } catch (error) {
