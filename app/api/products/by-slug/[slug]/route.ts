...
export async function GET(
  ...
  try {
    ...
    // Tìm sản phẩm trực tiếp trong database với cả slug gốc và normalized
    const rows = await executeQuery<ProductRow[]>(`
      ...
      AND (
        ...
        OR LOWER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
          p.slug, 'á', 'a'), 'à', 'a'), 'ả', 'a'), 'ã', 'a'), 'ạ', 'a'
        )) = ?
        OR LOWER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
          p.name, ' ', '-'), 'á', 'a'), 'à', 'a'), 'ả', 'a'), 'ã', 'a'), 'ạ', 'a'), 'ă', 'a'), 'ắ', 'a'), 'ằ', 'a'), 'ẳ', 'a'), 'ẵ', 'a'), 'ặ', 'a'
        )) = ?
      )
      LIMIT 1
    `, [slug, normalizedSlug, normalizedSlug])
