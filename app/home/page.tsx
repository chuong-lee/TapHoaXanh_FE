...
export default function HomePage() {
  ...
  return (
    <div className="home-page min-vh-100 bg-light">
      {/* Banner/Hero Section */}
      <section className="hero-section-green py-5 mb-4" style={{background: '#fff', borderRadius: '0 0 32px 32px'}}>
        <div className="container">
          <div className="row align-items-center">
            <div className="col-lg-7 mb-4 mb-lg-0">
              <div className="mb-2">
                <span className="badge bg-danger bg-opacity-10 text-danger px-3 py-2 fw-bold" style={{fontSize: '1rem'}}>100% Organic Vegetables</span>
              </div>
            ...
            <div className="col-lg-5 text-center">
              <Image
                ...
                width={480}
                height={400}
                className="rounded-4 shadow-lg hero-img"
        ...
        <div className="categories-scroll d-flex flex-nowrap gap-3">
          {categories.map((cat, idx) => (
            ...
            >
              ...
              <div className="text-secondary small">{cat.count} items</div>
            </div>
          ))}
        </div>
      </section>

      {/* Banner + Featured Products Section (giống HTML mẫu đầu tiên) */}
      <section className="container mb-5">
        <div className="featured-banner-products d-flex align-items-stretch" style={{gap: '12px'}}>
          ...
          <div className="featured-banner bg-dark text-white rounded-4 p-4 d-flex flex-column justify-content-between h-100" style={{minWidth: 260, maxWidth: 320}}>
            <div>
              <span className="badge bg-success mb-2">Banner</span>
              <h3 className="fw-bold mb-3" style={{fontSize: '2rem'}}>Bring nature<br/>into your home</h3>
            </div>
            <button className="btn btn-danger fw-bold px-4 py-2 mt-3 align-self-start" style={{borderRadius: 24, fontSize: 18}}>Shop Now</button>
          </div>
          ...
          <div className="featured-product-list d-flex flex-row align-items-stretch">
            {products.slice(0, 4).map((product, idx) => (
              ...
              >
                ...
                {/* Nhãn giảm giá */}
                {idx === 0 && <span className="badge bg-success position-absolute top-0 start-0 m-2" style={{fontSize:13, borderRadius:8}}>Save 35%</span>}
                {idx === 1 && <span className="badge bg-primary position-absolute top-0 start-0 m-2" style={{fontSize:13, borderRadius:8}}>Sale</span>}
                {idx === 2 && <span className="badge bg-warning text-dark position-absolute top-0 start-0 m-2" style={{fontSize:13, borderRadius:8}}>Best sale</span>}
                {idx === 3 && <span className="badge bg-danger position-absolute top-0 start-0 m-2" style={{fontSize:13, borderRadius:8}}>Save 15%</span>}
                {/* Brand + icon sao */}
                ...
                <div className="mb-2">
                  <div className="progress" style={{height: 7, borderRadius: 8}}>
                  ...
                  <div className="small text-muted mt-1">Sold: 90/120</div>
                </div>
                {/* Nút Add To Cart */}
                <button 
      ...
      {/* Popular Products */}
      <section className="container mb-5">
        <h2 className="fw-bold mb-4" style={{fontSize: '2rem'}}>Popular Products</h2>
        {loading ? (
            ...
            {currentProducts.map((product, idx) => (
              <div className="custom-product-card" key={product.id}>
                ...
                <div className="product-info">
                  <div className="product-type">Snack</div>
      ...
      <section className="container mb-5">
        <div className="row g-4 home-below-banners">
          <div className="col-md-6">
            ...
            >
              <span style={{background:'#dc3545',color:'#fff',fontWeight:700,fontSize:22,padding:'8px 28px',borderRadius:12,marginBottom:24,display:'inline-block'}}>Flat 20% Discount</span>
              <h2 style={{fontWeight:800,fontSize:'2.3rem',color:'#222',marginBottom:24,lineHeight:1.1}}>Purely Fresh<br/>Vegetables</h2>
              <button style={{background:'#ffc300',color:'#222',fontWeight:700,border:'none',borderRadius:10,padding:'12px 32px',fontSize:20,boxShadow:'0 2px 8px rgba(220,53,69,0.08)'}}>Shop Now</button>
            </div>
          </div>
          <div className="col-md-6">
            <div
            ...
            >
              <span style={{background:'#dc3545',color:'#fff',fontWeight:700,fontSize:22,padding:'8px 28px',borderRadius:12,marginBottom:24,display:'inline-block'}}>Flat 25% Discount</span>
              <h2 style={{fontWeight:800,fontSize:'2.3rem',color:'#222',marginBottom:24,lineHeight:1.1}}>Fresh Fruits<br/>Pure Qualitty</h2>
              <button style={{background:'#ffd43b',color:'#222',fontWeight:700,border:'none',borderRadius:10,padding:'12px 32px',fontSize:20,boxShadow:'0 2px 8px rgba(220,53,69,0.08)'}}>Shop Now</button>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="featured-product-list">
            ...
            {products.slice(rowIdx * 4, rowIdx * 4 + 4).map((product, idx) => (
              <div className="featured-product-card" key={product.id}>
                ...
                <div className="sold-row">Sold: 90/120</div>
